...
> 🔴 **遥控器待查（2026-09-11）**：C3 插在遥控器板上、USB 供电，三个键**没按就全读 0**：
> ```
> rc: 键电平 抬(GPIO4)=0 落(GPIO3)=0 复位(GPIO1)=0，0=按下
> ```
> 后果是遥控器一醒就以为抬+落都按着 → 等 3s 发配对 → 等松手 8s 超时 → 睡 → 立刻又醒，
> **每 11.4s 循环发一次配对广播**（主机日志里 `已配对` 每 11.4s 一条就是这个）。
> 电池插没插无关（按键接的 N5 GND 就是 C3 自己的 GND）。
> 三个一起错 = 同一个错误复制三次，最大嫌疑是**轻触开关内部相通的脚对与封装假设不符**
> （网表基准 §86 按"1/3 相通、2/4 相通、1-2 跨开关"画的；实物若是 1/2 相通，1-2 就永远短路）。
> **下一步**：C3 拔掉，万用表通断档量每颗开关四脚两两之间**不按**时哪对通 ——
> 信号和 GND 必须跨在**不相通**的两脚之间。
