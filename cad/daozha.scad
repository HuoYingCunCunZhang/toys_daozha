...
RC_HD_H = (RC_H - RC_TOP_T) - RC_PCB_Z;   // 压条长度 23 - 8.7 = 14.3
...
/* 🔴 板压条 (rc_top 背面), v3.3 新增。
   原来板子**只被托住、没被压住**: 下面靠四根柱子的 φ6 承托肩(顶面 z=7.1)撑着,
   四根 φ4.4 上段穿过板边缺口做横向定位 —— 但**往上什么都没有**。
   板面 8.7、上盖内表面 23, 中间 14.3mm 全空; 板子能往上窜到 U1 叠高顶(20.4)
   撞上盖为止, 即 **2.6mm**。后果不只是哐当响:
   板子一抬, 开关本体跟着抬, 顶杆把按键帽往上顶到法兰坐死, 再抬就是**误触发**。
   解法是从上盖背面伸四条压条下来, 把板子夹在"下面承托肩 + 上面压条"之间。
   ⚠ 用**条**不用柱: +Y 边只有 y 12…15 这 3mm 空当(按键本体到 y=12, 板边 15),
     圆柱最大只能 φ3 且余量为零; 做成 8×2.6 的条, y 向仍是 2.6, x 向拉长 8mm,
     强度和贴合面积都够, 而 14.3mm 长的 φ3 细柱又细又软。
   ⚠ 压条和按键导向凸台重叠不要紧 —— **两个都是上盖上的料, 融在一起反而更结实**。
     真正要让开的只有: 板上的元件、板边缺口、板子外形、**按键帽的下柄**。
   ⚠ 按键帽的下柄容易漏: 帽的锥柄在 z 16.9…20 撑到 φ9, 而上盖的 φ9.5 沉孔
     只切 z≥20 那一段, 柄那一段没被切掉 —— 压条压过去就顶住帽子。自检 t=68 抓到的,
     第一版把压条放在 (-16,13.5) 正是按键帽正上方。
   位置原则: **每条都贴着一个承托肩(板边缺口)的正上方** —— 压点正对支点, 板不受弯。
   +Y 边 -X 半场被 SW4 + 两颗按键帽的柄 + 缺口占满, 只剩 x -4.7…-2.7 这 2mm,
   所以第 4 条只有 2.0 长; 它力不大, 任务只是别让那个角翘起来。          */
RC_HD_W = 2.6;      // y 向宽 (+Y 边只有 y 12…15 这 3mm 空当)
// [x, y, x向长] —— PCB 坐标。长度逐条不同, 因为四处的空当宽度不一样
RC_HD = [[-7.5, -16.5, 8.0],   // 贴 (-14,-17.2) 那个缺口
         [ 7.5, -16.5, 8.0],   // 贴 ( 14,-17.2)
         [ 18.0, 13.5, 8.0],   // 贴 ( 11, 14.2)
         [ -3.7, 13.5, 2.0]];  // 贴 (-7.5, 14.2), 夹在缺口和按键帽柄之间
...
module rc_top() {
    difference() {
        union() {
            ...
            // 板压条: 从上盖背面(本地 z=0)一路伸到板面(绝对 8.7)
            for (p = RC_HD)
                translate([rcx(p)[0] - p[2]/2, rcx(p)[1] - RC_HD_W/2, -RC_HD_H])
