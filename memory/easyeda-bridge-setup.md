...
- **覆铜范围读 `pcb_PrimitivePour.getAll()` 的 `complexPolygon`**：用户画的区域，常常就是一句矩形模式 `["R", x, y, w, h, rot, round]`（x/y 是**左上角**，单位 mil），简单可靠。

🔴 **`pcb_PrimitivePoured` 的 `pourFills` 坐标单位是 10 mil（1 单位 = 0.254mm），不是 mil。** 2026-09-13 查遥控器时解开的——这正是当初"解析出来的包围盒只有 11.5×5.8mm 而板子是 120×65"的原因：**差的就是这 10 倍**（120×65mm = 4724×2559 mil，÷10 再当 mil 换算正好 12.0×6.5mm）。按 10 mil 解出来的遥控器填充包围盒是 54.78×29.14mm，对上覆铜区域 54.99×29.34mm，四周各内缩 0.1mm = 隔离间距，自洽。

`pourFills[i].path.complexPolygon` 是**路径数组**（外轮廓 + 各个避让孔），每条路径是扁平数组、夹着 `"L"` 和 `"ARC"` 记号；`ARC` 的格式是 `"ARC", 角度, endX, endY`（**与格式文档正文一致、与文档示例不一致，示例是错的**）。

**用它能算出 `pcb_Drc` 查不出来的东西**：覆铜对某条走线/某个焊盘的实际净距。判据要**减掉线宽的一半或焊盘半径**——焊盘是 `["ELLIPSE",70.9,70.9]` mil 即 φ1.80mm，"到焊盘中心 1.258mm"其实是净距 0.358mm。**必须带对照**：GND 网络自己的走线应该算出**负数**（重叠，因为它就在 GND 覆铜里），算不出负数说明解析错了。

**解析完仍然必须拿包围盒对一下板子大小**，对不上就是解错了，别拿错的数下结论。
